...
class Attention(nn.Module):
    def __init__(self, dim=64, num_heads=8, bias=False):
        ...
        self.kv = nn.Conv2d(self.dim, self.dim * 2, kernel_size=1, bias=bias)
        self.kv_dwconv = nn.Conv2d(self.dim * 2, self.dim * 2, kernel_size=3, stride=1, padding=1, groups=self.dim * 2, bias=bias)
        self.q = nn.Conv2d(self.dim, self.dim, kernel_size=1, bias=bias)
        self.q_dwconv = nn.Conv2d(self.dim, self.dim, kernel_size=3, stride=1, padding=1, groups=self.dim, bias=bias)
    ...
    def forward(self, x, y):
        ...
        assert self.dim % self.num_heads == 0, "channels must be divisible by num_heads"
...
class FuseBlock(nn.Module):
    def __init__(self, spacial_channels, frequency_channels, num_heads=8, fourier_dim=(-2, -1)):
        ...
        # Convolutional preprocessing for spatial_fature and frequency_feature
    ...
    def forward(self, spatial_feature, frequency_feature):
        ...
        # Preprocess spatial_feature and frequency_feature with convolutions
        spatial_feature = self.spatial_conv(spatial_feature)
        frequency_feature = self.frequency_conv(frequency_feature)
        
        # FFT both spatial_feature and frequency_feature
        spatial_fft = torch.fft.fftn(spatial_feature, dim=self.fourier_dim)
        frequency_fft = torch.fft.fftn(frequency_feature, dim=self.fourier_dim)
        ...
        # al_spatial = spatial_fft.real.contiguous()
