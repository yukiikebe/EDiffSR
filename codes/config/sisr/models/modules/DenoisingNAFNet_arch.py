...
class ConditionalNAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], upscale=1, uno=None):
        ...
        self.FuseBlocks = nn.ModuleList([
            FuseBlock(channels = 64, num_heads=8, fourier_dim=(-2, -1)),
            FuseBlock(channels = 128, num_heads=8, fourier_dim=(-2, -1)),
            FuseBlock(channels = 256, num_heads=8, fourier_dim=(-2, -1)),
            FuseBlock(channels = 512, num_heads=8, fourier_dim=(-2, -1)),
        ])
    ...
    def forward(self, inp, cond, time):
        ...
        for idx, (encoder, down) in enumerate(zip(self.encoders, self.downs)):
            x, _ = encoder([x, t])
            if self.uno is not None:
                # print("uno_features shape", uno_features[idx].shape)
                ...
                # save_dir = "/home/yuki/research/EDiffSR_combine_DifGaussian/debug"
