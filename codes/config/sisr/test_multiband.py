    ...
    for i, test_data in enumerate(test_loader):
        ...
        if output_np.shape[0] == 7:
            ...
            with rasterio.open(save_img_path) as src:
                ...
                assert src.count == 7
                output_np = src.read([1,2,3])
                output_np = np.moveaxis(output_np, 0, -1)

                if output_np.dtype != np.uint8:
                    img_min, img_max = output_np.min(), output_np.max()
                    if img_max > img_min:
                        output_np = ((output_np - img_min) / (img_max - img_min) * 255).astype(np.uint8)
                    else:
            ...
            if suffix:
                ...
                output_np = np.moveaxis(output_np, 0, -1)
                output_np = (output_np * 255).round().clip(0, 255).astype(np.uint8)
                save_img_path = os.path.join(dataset_dir, img_name + ".png")
            util.save_img(output_np, save_img_path)
