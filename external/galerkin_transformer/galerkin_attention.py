...
class simple_attn(nn.Module):
    ...
    def forward(self, x, name='0'):
        ...
        qkv = self.qkv_proj(x).permute(0, 2, 3, 1).contiguous().reshape(B, H*W, self.heads, 3*self.headc)
        qkv = qkv.permute(0, 2, 1, 3).contiguous()
        q, k, v = qkv.chunk(3, dim=-1)
        
        # k = k.view(B * self.heads, H * W, self.headc)
        # v = v.view(B * self.heads, H * W, self.headc)
        ...
        v = torch.matmul(q, v)
        v = v.permute(0, 2, 1, 3).contiguous().reshape(B, H, W, C)

        ret = v.permute(0, 3, 1, 2).contiguous() + bias
