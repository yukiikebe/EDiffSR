...
# ####
class UNO(nn.Module):
    def __init__(self, in_width, width, pad=0, factor=3 / 4, region=None):
        ...
        # self.fc2 = nn.Conv2d(4 * self.width, 8, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x_c0 = self.L0(x_fc0, int(D1 * self.factor), int(D2 * self.factor), region=self.region)
        
        x_c1 = self.L1(x_c0, D1 // 2, D2 // 2, region=self.region)

        x_c2 = self.L2(x_c1, D1 // 4, D2 // 4, region=self.region)
        x_c3 = self.L3(x_c2, D1 // 4, D2 // 4, region=self.region)
        x_c4 = self.L4(x_c3, D1 // 2, D2 // 2, region=self.region)
        x_c4 = torch.cat([x_c4, x_c1], dim=1)
        x_c5 = self.L5(x_c4, int(D1 * self.factor), int(D2 * self.factor), region=self.region)
        x_c5 = torch.cat([x_c5, x_c0], dim=1)
        x_c6 = self.L6(x_c5, D1, D2, region=self.region)
        x_c6 = torch.cat([x_c6, x_fc0], dim=1)
