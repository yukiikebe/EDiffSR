...
class UNO(nn.Module):
    def __init__(self, in_width, width, pad=0, factor=3 / 4):
        ...
        self.L0 = OperatorBlock_2D(self.width, int(2 * factor * self.width), 48, 48, 22, 22)

        self.L1 = OperatorBlock_2D(
            int(2 * factor * self.width), int(4 * factor * self.width), 32, 32, 14, 14
        )

        self.L2 = OperatorBlock_2D(
            int(4 * factor * self.width), int(8 * factor * self.width), 16, 16, 6, 6
        )

        self.L3 = OperatorBlock_2D(
            int(8 * factor * self.width), int(8 * factor * self.width), 16, 16, 6, 6
        )

        self.L4 = OperatorBlock_2D(
            int(8 * factor * self.width), int(4 * factor * self.width), 32, 32, 6, 6
        )

        self.L5 = OperatorBlock_2D(
            int(8 * factor * self.width), int(2 * factor * self.width), 48, 48, 14, 14
        )
    ...
    def gen_feat(self, x):
        self.inp = x  
        grid = self.get_grid(x.shape, x.device)
        x = torch.cat((x, grid), dim=-1)  # [B, H, W, in_width + 4]
        ...
        x_fc0 = x_fc0.permute(0, 3, 1, 2)  # [B, C, H, W]
        ...
        self.feat = x_c6  # [B, C, H, W]
        return self.feat

    def query_features(self, coord):
        """
        coord: [B, H, W, 2] in [-1, 1]
        """
        feat = self.feat  # [B, C, H, W]
        x = F.grid_sample(feat, coord.flip(-1), mode='bilinear', align_corners=True)
        x = F.gelu(self.fc1(x))
        x = self.fc2(x)  # [B, 1, H, W]

        # optional: add original input sampling
        base = F.grid_sample(
            self.inp.permute(0, 3, 1, 2),
            coord.flip(-1),
            mode='bilinear',
            padding_mode='border',
            align_corners=True
    ...
    def forward(self, x, coord=None, return_feat=False):
        feat = self.gen_feat(x)
        if return_feat:
            return feat
        elif coord is not None:
            return self.query_features(coord)
        else:
            raise ValueError("coord must be provided unless return_feat=True")
...
class UNO_HiLoc(nn.Module):
    def __init__(self, in_width, width, pad=0, factor=3 / 4, layers=8, debug=False):
        ...
        for i in range(self.layers + 1):
            if i < self.layers // 2:
                ...
                exp_index = self.layers - i - 1
            channel = int(4 * (2 ** exp_index) * factor * self.width)

            if i == 8:
                self.galerkin_encoders.append(
                    simple_attn(channel, heads=8, output_dim=self.width)
                )
            else:
                self.galerkin_encoders.append(
                    simple_attn(channel, heads=8)
                )
        ...
        self.L5_whole, self.L5_patches = make_Layer(self.width, self.factor, 16, 8, 16, 16, 4, 4)
        self.L6_whole, self.L6_patches = make_Layer(self.width, self.factor, 16, 4, 32, 32, 6, 6)
        self.L7_whole, self.L7_patches = make_Layer(self.width, self.factor, 8, 2, 48, 48, 14, 14)
        self.L8_whole, self.L8_patches = make_Layer(self.width, self.factor, 4, 1, 64, 64, 22, 22)
        ...
        self.grid_whole = None  
        self.grid_patches = {}
        self.pos_enc_cache = {}
        self.grid_patch_cache = {}
        self.encoder_features = False
    ...
    def forward(self, x_whole):
        ...
        for i in range(self.layers):
            ...
            x_c_mirrored = None
            x_c = None
            if index > self.layers // 2 and index < self.layers:
                mirrored_index = self.layers - index - 1
                x_c_mirrored = result_features[mirrored_index]
        ...
        if self.padding != 0:
            result_features[-1] = result_features[-1][..., :-self.padding, :-self.padding]
        
        #linear
        # result_features[-1] = result_features[-1].permute(0, 2, 3, 1)  # [B, H, W, C]
        
        # x_fc1 = self.fc1(result_features[-1])
        # x_fc1 = F.gelu(x_fc1)
        
        #conv
        x_fc1 = self.fc1(result_features[-1])  # [B, C, H, W]
        x_fc1 = F.gelu(x_fc1)
        
        x_out = self.fc2(x_fc1)  # [B, H, W, 3]
            
        self.num_iter += 1
        
        if self.encoder_features:
            return result_features[:4], x_out  
        else:
            return x_out
    ...
    def process_integral_operator_patches(self, L_patches, x_fc0_patches, dim1, dim2):
        ...
        return list(torch.chunk(x_c_tensor, len(x_fc0_patches), dim=0))  
    ...
    def process_layer(self, index, x_c_prev, D1_whole, D2_whole, D1_patch, D2_patch, x_c_mirrored=None, x_fc0=None):
        ...
        if index < self.layers // 2:
            exp_index = index
        elif index <= self.layers// 2 + 2 and index >= self.layers // 2:
            exp_index = self.layers - index - 1
        
        if index == 7:
            x_c_whole = L_whole(x_c_prev, int(D1_whole * self.factor), int(D2_whole * self.factor))
            
            L_patches = getattr(self, f"L{index}_patches")
            x_c_patch = self.process_integral_operator_patches(L_patches,
                                                               x_c_patches,
                                                               int(D1_patch * self.factor),
                                                               int(D2_patch * self.factor))
        elif index == 8:
            x_c_whole = L_whole(x_c_prev, D1_whole, D2_whole)
            ...
            x_c_whole = L_whole(x_c_prev, D1_whole // (2**(exp_index)), D2_whole // (2**(exp_index)))
            
            dim1 = int(D1_patch // (2**(exp_index)))
            dim2 = int(D2_patch // (2**(exp_index)))
        ...
        if x_c_mirrored is not None:
            x_c = torch.cat([x_c, x_c_mirrored], dim=1)  
        
        if x_fc0 is not None:
            x_c = torch.cat([x_c, x_fc0], dim=1)  
